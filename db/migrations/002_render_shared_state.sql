-- Render PostgreSQL migration for the shared production snapshot and audit trail.
-- The existing 001_init.sql remains preserved for Supabase-compatible relational deployments.
...
values (1, jsonb_build_object('people', '[]'::jsonb, 'locations', '[]'::jsonb,
  'schedules', '[]'::jsonb, 'dailyPlans', '[]'::jsonb, 'suggestions', '[]'::jsonb,
  'costPerKm', 1.2), 1, 'database-bootstrap', 'ONLINE')
...
create policy app_state_api_access on public.app_state for all
...
create policy app_state_audit_api_access on public.app_state_audit for all
  using (current_setting('app.access_granted', true) = 'true')
  with check (current_setting('app.access_granted', true) = 'true');
drop policy if exists settings_api_access on public.settings;
create policy settings_api_access on public.settings for all
  using (current_setting('app.access_granted', true) = 'true')
  with check (current_setting('app.access_granted', true) = 'true');
